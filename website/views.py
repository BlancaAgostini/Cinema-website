...
import stripe
from .models import Cinemahall, Movie, Show, User, Booking, Showseat
from . import db
...
def success():
    if request.method == 'GET':
        ...
        return render_template("success.html", session=session, show_of_movie=show_of_movie, movie=movie_to_display, customer=customer)
...
def main_movie_shows():
    movie = Movie.query.filter_by(id=7).first()
